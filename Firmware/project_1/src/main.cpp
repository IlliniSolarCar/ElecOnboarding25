...
int main() {
	...
	while (!shutdown) {
        ...
        if(timing.tickThreshold(last_task_1_time, RATE)){
        	...
        	if(led5.read()) led5.write(0);
        	else led5.write(1);
        	//not really a way to test or debug this
        	//but this should toggle led5 on every RATE microseconds
        	//im pretty sure i did everything right idk
